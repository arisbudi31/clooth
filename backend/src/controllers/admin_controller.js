...
module.exports.addAdmin = async (req, resp) => {
  ...
  try {
    ...
    //send respond to client
    const respond = new createRespond(statusCode.CREATED, statusMessage.CREATED, 'Add Admin', 1, 1, Info)
    resp.status(respond.status).send(respond.data)
...
module.exports.ReportBy = async (req, resp) => {
  ...
  try {
    ...
    //get report period
    const getReportsBy = `SELECT sum(det.reservedStock) AS totalSoldProduct, SUM(det.priceAtTime*det.reservedStock) AS totalRevenue, WEEK(pay.createdAt) AS weeks, MONTH(pay.createdAt) AS month
        FROM invoice_details AS det
...
module.exports.TopThree = async (req, resp) => {
  ...
  try {
    ...
    const getTop3 = `SELECT det.productId AS productId, pd.productName, pd.price, SUM(det.reservedStock) AS qtyOrder
    FROM invoice_details AS det
    JOIN products AS pd ON pd.id = det.productId
    GROUP BY productId
